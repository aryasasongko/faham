...
const LOCALE = { en: 'en-GB', id: 'id-ID', ko: 'ko-KR' };
...
export function formatClock(date, zone) {
  ...
  /* A moment is absolute; the wall clock it should be read on is not. When the
     times belong to a city the user picked rather than to the device's own
     position, they must be shown on that city's clock. */
  if (zone) {
    try {
      return new Intl.DateTimeFormat('en-GB', {
        timeZone: zone, hour: '2-digit', minute: '2-digit', hour12: false
      }).format(date);
    } catch (e) { /* unknown zone — fall through to the device clock */ }
  }
