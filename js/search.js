...
function run() {
  ...
  /* Korean counters attach straight to the numeral (6개, not 6 개), and the
     query reads better fronted: “수주드” 검색 결과 6개. */
  const heading = (state.language === 'ko')
    ? '“' + box.value + '” 검색 결과 ' + hits + '개'
    : hits + ' ' + P('match' + (hits === 1 ? '' : 'es') + ' for', 'hasil untuk') +
      ' “' + box.value + '”';
  setHTML($('results'),
