...
import { P, t, pairBlock, isPair } from '../i18n.js';
import { state } from '../state.js';
import { $, esc, setHTML, delegate } from '../dom.js';
import { deeper, whyNote, paragraphs, untranslatedNote } from './common.js';
import { playButton, reciterSwitch, vocabAudioId } from '../audio.js';

let openStoryIndex = null;

export function renderConcepts() {
  setHTML($('concepts'), untranslatedNote() + CONCEPTS.map((c) => (
    '<div class="card srch"><div class="card-h"><h3>' + P(c.t, c.tid || c.t, c.tko) + '</h3></div>' +
    '<p class="en" style="margin-top:9px">' + P(c.s, c.sid, c.sko) + '</p>' + pairBlock(c.s, c.sid) +
    deeper(c.l, c.lid, c.lko) + '</div>'
  )).join(''));
}

export function renderIslam() {
  setHTML($('islamList'), untranslatedNote() + ISLAM.map((c) => (
    '<div class="card srch"><div class="card-h"><h3>' + P(c.t, c.tid, c.tko) + '</h3></div>' +
    '<p class="isl-sub">' + P(c.sub, c.subid, c.subko) + '</p>' +
    '<p class="en" style="margin-top:11px">' + P(c.s, c.sid, c.sko) + '</p>' + pairBlock(c.s, c.sid) +
    deeper(c.l, c.lid, c.lko) + '</div>'
  )).join(''));
}
...
export function renderStories() {
  setHTML($('storyIndex'), untranslatedNote() + STORIES.map((s, i) => (
    '<button type="button" class="story-row srch" data-story="' + i + '">' +
    '<span class="mins">' + esc(P(s.mins, s.minsid, s.minsko)) + '</span>' +
    '<span class="st"><span class="story-h">' + P(s.t, s.tid, s.tko) + '</span>' +
    '<span class="story-p">' + P(s.hook, s.hookid, s.hookko) + '</span>' +
    '<span class="ref">' + P(s.ref, s.refid, s.refko) + '</span></span></button>'
  )).join(''));
...
export function openStory(i) {
  ...
  const source = P(s.p, s.pid, s.pko);
  ...
  const verses = s.v.map((v) => (
    ...
    '<p class="en">' + P(v.en, v.id, v.ko) + '</p>' + pairBlock(v.en, v.id) + '</div>'
  )).join('');

  const reader = $('reader');
  setHTML(reader,
    '<button class="back" type="button" data-story-back="1">' + esc(t('back')) + '</button>' +
    '<article class="read"><h3>' + P(s.t, s.tid, s.tko) + '</h3>' +
    '<p class="meta">' + esc(P(s.mins, s.minsid, s.minsko)) + ' · ' + esc(P(s.ref, s.refid, s.refko)) + '</p>' +
    '<div class="prose">' + prose + '</div>' +
    pairBlock(paragraphs(s.p), paragraphs(s.pid), true) + verses +
    whyNote('howtold', s.why, s.whyid, s.whyko) + '</article>');
  ...
  const html = reciterSwitch(P(RECITER_NOTE[0], RECITER_NOTE[1])) + SURAHS.map((s) => {
    const verses = s.v.map((v, i) => {
      ...
      return '<div class="vrow">' +
        '<div class="vhead">' + playButton(id, 'quran', [s.no + ':' + (i + 1)],
          { small: true, name: s.name + ', ' + t('verse_n').replace('{n}', i + 1) }) +
    ...
    return '<div class="card srch"><div class="card-h"><h3>' + esc(s.name) + '</h3>' +
      '<span class="pos">' + esc(P(s.meta, s.metaid, s.metako)) + '</span></div>' +
      '<p class="en" style="margin-top:7px">' + P(s.intro, s.introid, s.introko) + '</p>' + pairBlock(s.intro, s.introid) +
      '<div class="vhead-all">' +
      playButton('surah' + s.no, 'quran', s.v.map((v, i) => s.no + ':' + (i + 1)),
        { name: s.name }) +
      '<span>' + esc(t('playWhole')) + '</span></div>' + verses +
      whyNote('doing', s.why, s.whyid, s.whyko) + '</div>';
  }).join('');
  setHTML($('surahs'), html);
}
...
export function renderVocab() {
  setHTML($('vocabList'), untranslatedNote() + VOCAB.map((v, i) => {
    const audio = playButton(vocabAudioId(v), 'local', null, { small: true, name: v.tl });
    return '<div class="word srch" data-word="' + i + '">' +
      ...
      '<span class="w-en">' + esc(P(v.en, v.id, v.enko)) + '</span>' +
      (isPair() ? '<span class="w-id">' + esc(state.language === 'en' ? v.id : v.en) + '</span>' : '') +
      '<span class="w-note">' + esc(P(v.noteen, v.note, v.noteko)) + '</span></span></button>' +
