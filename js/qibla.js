...
export const compass = {
  ...
  status: 'idle',
  /* 'unknown' until enough samples; then 'steady' | 'unsteady'. A phone that
     needs calibrating swings by tens of degrees while lying still, and the
     honest thing is to say so rather than draw a confident needle. */
  quality: 'unknown'
};

const smooth = { x: 0, y: 0, has: false };
/* Rolling spread of the raw heading, used only to judge whether the reading is
   stable enough to trust. Circular, so 359°→1° counts as 2° of movement. */
const jitter = { samples: [], spread: 0 };

function noteJitter(raw) {
  jitter.samples.push(raw);
  if (jitter.samples.length > 24) jitter.samples.shift();
  if (jitter.samples.length < 12) { compass.quality = 'unknown'; return; }
  let sx = 0, sy = 0;
  jitter.samples.forEach((d) => { sx += Math.cos(toRad(d)); sy += Math.sin(toRad(d)); });
  const n = jitter.samples.length;
  const r = Math.sqrt(sx * sx + sy * sy) / n;          // 1 = perfectly steady
  jitter.spread = toDeg(Math.sqrt(Math.max(0, -2 * Math.log(Math.max(r, 1e-6)))));
  compass.quality = jitter.spread > 12 ? 'unsteady' : 'steady';
}
let attached = false;
let frame = null;
let onUpdate = null;
