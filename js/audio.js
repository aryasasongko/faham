...
import { AUDIO_BASE, AUDIO_MAP, AUDIO_AVAILABLE, VOCAB_AUDIO_DIR } from '../data/audio-map.js';
...
/* Filenames that exist in assets/audio/ right now. Anything not in here has no
   control rendered — see AUDIO_AVAILABLE in data/audio-map.js. */
const SHIPPED = new Set(AUDIO_AVAILABLE);

/**
 * Local (non-Quran) id → path, honouring a school-specific recording.
 * Returns null when the clip has not been recorded yet, which is what stops a
 * dead button being drawn. "Audio unavailable" is then reserved for a file that
 * was supposed to be there and failed to load — a genuine fault, not an
 * expected gap.
 */
function localUrl(id) {
  let file;
  if (id.indexOf('vocab:') === 0) {
    file = VOCAB_AUDIO_DIR + id.slice(6) + '.mp3';
  } else {
    const entry = AUDIO_MAP[id];
    if (!entry) return null;
    const variant = entry.variants && entry.variants[currentKey()];
    file = variant || entry.file;
  }
  return SHIPPED.has(file) ? AUDIO_BASE + file : null;
}
...
export function playButton(id, kind, keys, opts) {
  ...
  /* `name` makes the accessible label unique. Thirty buttons all announcing
     "Play recitation" is unusable on a screen reader; "Play recitation,
     Al-Fatihah verse 3" is what a listener actually needs. It is stored on the
     element so paintButtons can rebuild the label as the state changes. */
  const name = options.name || '';
  const base = options.label || (kind === 'quran' ? t('aud_playrec') : t('aud_play'));
...
export function paintButtons(failedId) {
  qsa('.playb').forEach((b) => {
    ...
    const name = b.dataset.audioName || '';
    const failed = b.getAttribute('data-failed') === '1';
    const base = loading ? t('aud_loading')
      : failed ? t('aud_missing')
      : isActive ? t('aud_stop')
      : (b.dataset.audioKind === 'quran' ? t('aud_playrec') : t('aud_play'));
    b.setAttribute('aria-label', name ? base + ', ' + name : base);
  });
}
